...
void AHelicalFence::FenceHidden()
{
	...
	FGraphEventRef HiddenTask = FFunctionGraphTask::CreateAndDispatchWhenReady([this]()
	{
		...
		TArray<TObjectPtr<ASingleFence_Base>> FenceSplineAllSingleFences = FenceSpline->GetAllSingleFences(); // 获取围栏曲线的所有围栏对象

		if (FenceSplineAllSingleFences.IsEmpty()) return;
		...
		for (auto& SingleFences : AllSingleFences)
		{
			if (SingleFences && SingleFences->GetFenceMesh())
			{
				...
				if (FenceSplineAllSingleFences[Index])
				{
					FenceSplineAllSingleFences[Index]->SetActorHiddenInGame(!Hidden);
				}
